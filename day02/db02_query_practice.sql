...
-- > 미만 < 초과 >= 이하 <= 이상 <> 같지않다 != 같지않다
SELECT *
  FROM Book
 WHERE price < 20000;
 
-- 3-5 : 가격이 10000원에서 20000원 사이인 이하인 도서를 검색하시오.
SELECT *
  FROM Book
...
-- 3-6, 3-11 : 출판사가 '굿스포츠' 또는 '대한미디어'인 도서를 검색하시오.alter
SELECT *
  FROM Book
...
SELECT *
 ...
 SELECT publisher
  ...
  -- 		_구% : 첫번째 글자는 뭐든지 상관없고, 두번째 글자가 구로 되고 나머지 뒤의 글이 더있다
 SELECT *
   FROM Book
