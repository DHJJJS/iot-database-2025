...
-- 그룹화 GROUP BY 키워드 사용
-- 3-19 고객별로 주분한 도서의 총수량과 총판매액을 구하시오.
-- GROUP BY를 사용하면 반드시 집계함수를 SELECT 안에 포함시켜야.
SELECT custid
...
SELECT *
  FROM Customer
 INNER JOIN Orders
    ON Customer.cusid = Orders.cusid;
...
SELECT *
  FROM Customer AS c
  LEFT JOIN Orders AS o -- LEFT OUTER JOIN Order as o
	ON c.custid = o.custid;
    
-- right outer joing으로 하면?
SELECT *
  FROM Customer AS c
 RIGHT JOIN Orders AS o -- LEFT OUTER JOIN Order as o
	ON c.custid = o.custid;
