...
def showDatas():
    ...
    data = cursor.fetchall()        # 쿼리 실행 데이터 모두 가져옴
...
def getData(event):
    ...
    if sel_item:
        item_value = dataView.item(sel_item[0], 'values')  # 선택항목 'values'(실데이터) 가져오기

    ## 엔트리 위젯에 각각 채워넣기
    ent1.config(state='normal')     # 데이터 들어갈 수 잇게 해주고
    ent1.insert(0, item_value[0])   # 학생번호
    ent1.config(state='readonly')   # 다시 리드온리로
    ent2.insert(0, item_value[0])   # 학생명
    ent3.insert(0, item_value[0])   # 핸드폰
    ent4.insert(0, item_value[0])   # 입학년도
...
def modData():
    ...
    sel_item = dataView.selection()
    if not sel_item:
        messagebox.showwarning("경고", "수정할 학생을 선택하세요!")
        return  # 선택된 항목이 없으면 함수 종료

    if sel_item:
        item_value = dataView.item(sel_item, 'values')  # 선택항목 'values'(실데이터) 가져오기
        ...
        try:
            ...
            messagebox.showinfo('INSERT', '학생수정 성공!')

        except Exception as e:
                print(e)
                conn.rollback() # 트랜잭션 롤백
                messagebox.showerror('INSERT', '학생수정 실패!!')
        finally:
                cursor.close()          # 커서
                conn.close()            # 커넥션 종료
